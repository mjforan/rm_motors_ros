...
int main() {
    ...
    for (int voltage = 0; voltage <= MAX; voltage += 2) {
        gm6020_can_set_cmd(gmc, ID, CmdMode::Voltage, voltage / 10.0);
    ...
    gm6020_can_set_cmd(gmc, ID, CmdMode::Voltage, 1.0);
    while (true){
        std::this_thread::sleep_for (std::chrono::milliseconds(50));
        std::cout<<gm6020_can_get_state(gmc, ID, FbField::Position)<<std::endl;
    }
